"""POS ramp LR searches: posA (tag side only) and rampB (tags -> words
from a fresh init).  Produces the pins."""
...
POS_A_SEARCH = [
    {"model": [MODEL_INIT_EXT],
     ...
     "training": [training(lr, total_dev=2_000_000, slurm_time=WALLTIME[size],
                           transition_schedule=POS_ONLY) for lr in LR_LADDER],
     "scheduler": [SCHEDULER],
...
RAMP_B_SEARCH = [
    {"model": [MODEL_PRETRAINED_EXT],
     "transition": [RESET],
     "dataset": [tag_stream(size)],
     "training": [training(lr, total_dev=5_000_000, slurm_time=WALLTIME[size],
                           transition_schedule=ramp(size)) for lr in LR_LADDER],
     "scheduler": [SCHEDULER],
...
EXPERIMENTS = {
    "pos_a_search":  {"stage_A": POS_A_SEARCH},
    "ramp_b_search": {"stage_A": [NO_STAGE_A], "stage_B": RAMP_B_SEARCH},
}
