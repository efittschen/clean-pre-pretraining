"""Epoched best x best: every prior at its pin into Stage B trained for
4 or 8 epochs."""
...
WALLTIME_A = {1_000_000: "00:20:00", 3_000_000: "00:20:00", 10_000_000: "00:30:00",
              30_000_000: "00:45:00", 100_000_000: "01:30:00"}

# Stage-A pins, copied from best_x_best/10_best_x_best.py (the reference copy).
STAGE_A_LR = {
...
# PINS from epoched/01_hp_search.py: (words, epochs, lr, wd).  Update here only.
EPOCHED_TARGETS = [
    (1_000_000,  4, 1e-3, 2.0),
    (1_000_000,  8, 1e-3, 5.0),
    (10_000_000, 4, 5e-4, 1.0),
    (10_000_000, 8, 1e-3, 2.0),
]
EPOCHED_WALLTIME = {1_000_000: "00:30:00", 10_000_000: "02:00:00"}
...
def training(lr, total_dev, slurm_time, wd=0.1, **extra):
...
STAGE_B = [
    {"model": [MODEL_PRETRAINED],
     "transition": [RESET],
     "dataset": [eng_b(words)],
     "training": [training(lr, total_dev=5_000_000, wd=wd, num_train_epochs=epochs,
                           slurm_time=EPOCHED_WALLTIME[words])],
     "scheduler": [SCHEDULER],
     "evaluation": [eval_config("lm_harness", tasks="blimp"),
                    eval_config("perplexity", split="dev")]}
    for (words, epochs, lr, wd) in EPOCHED_TARGETS]

EXPERIMENTS = {
    "epoched_best_x_best": {"stage_A": STAGE_A, "stage_B": STAGE_B},
}
